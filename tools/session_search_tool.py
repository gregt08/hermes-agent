...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "FTS5-backed SQLite retrieval — no LLM calls, every shape returns actual messages.\n\n"
        "THREE SHAPES:\n"
        "  1) DISCOVERY — pass `query`: FTS5 search returns top N sessions with snippets, "
        "bookend_start (first 3 msgs), messages window around match, bookend_end (last 3 msgs). "
        "Use `limit` (default 3, max 10), `sort` ('newest'/'oldest'), `role_filter`. "
        "AND is default; use OR/quotes/booleans/prefix wildcards (`deploy*`) in queries.\n\n"
        "  2) SCROLL — pass `session_id` + `around_message_id`: returns ±`window` msgs around "
        "the anchor. Forward: pass messages[-1].id; backward: pass messages[0].id.\n\n"
        "  3) BROWSE — no args: returns recent sessions chronologically (titles, previews, timestamps).\n\n"
        "Use for \"what did we do about X\" / \"where did we leave Y\" / \"find session Z\" — "
        "before gh, web search, or filesystem inspection. External tools show current world state; "
        "session DB shows what was said when."
    ),
    "parameters": {
        "type": "object",
        "properties": {
