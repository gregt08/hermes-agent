...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "Skills are reusable procedural memory — store recurring task approaches. "
        f"New skills go to {display_hermes_home()}/skills/.\n\n"
        "create: full SKILL.md + optional category. "
        "patch: old_string/new_string (preferred for fixes). "
        "edit: full SKILL.md rewrite (major overhauls only). "
        "delete: pass absorbed_into=<umbrella> to merge into another skill, or absorbed_into=\"\" to prune without forwarding. The target skill must already exist — create/patch it first, then delete the old one. "
        "Pinned skills are protected from deletion (unpin via `hermes curator unpin <name>`); patches/edits go through on pinned skills so you can improve pitfalls as they arise.\n\n"
        "Create when: complex task succeeded (5+ calls), errors overcome, user-corrected approach, non-trivial workflow. "
        "Update when: instructions stale/wrong, OS-specific failures, missing steps. "
        "Skip for simple one-offs. Confirm before creating/deleting.\n\n"
        "Good skills: trigger conditions, numbered steps, exact commands, pitfalls, verification steps. "
